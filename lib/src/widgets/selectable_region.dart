...
class SelectionChangedResult {
  ...
  List<TextSelectionPoint> selectionPoints;
  int lineStart;
  int lineEnd;
  int positionStart;
  int positionEnd;
...
class SelectableRegion extends StatefulWidget {
  ...
  final ValueChanged<SelectionChangedResult>? onSelectionChanged;
...
class SelectableRegionState extends State<SelectableRegion>
  ...
  void _updateSelectedContentIfNeeded() {
    if (_lastSelectedContent?.plainText !=
      ...
      final lineStart = _selectionDelegate.currentSelectionStartIndex;
      final lineEnd = _selectionDelegate.currentSelectionEndIndex;
      int positionStart = 0;
      if (_selectionDelegate._startHandleLayerOwner != null) {
        positionStart = (_selectionDelegate._startHandleLayerOwner!
                as TextSelectionGetterMixin)
            .textSelectionStart!
            .offset;
      }
      int positionEnd = 0;
      if (_selectionDelegate._endHandleLayerOwner != null) {
        positionEnd = (_selectionDelegate._endHandleLayerOwner!
                as TextSelectionGetterMixin)
            .textSelectionEnd!
            .offset;
      }
      ...
      List<Rect> selectionRects = [];
      for (final selectable in _selectionDelegate.selectables) {
        if (selectable is SelectableFragment) {
          final paragraph = selectable.paragraph;
          selectionRects.addAll(paragraph.selectionRects);
        }
      }
      ...
      if (_lastSelectedContent != null) {
        points = selectionEndpoints;
      }

      widget.onSelectionChanged?.call(SelectionChangedResult(
        selectedContent: _lastSelectedContent,
