...
class RenderParagraph extends RenderBox
  ...
  List<Rect> get selectionRects {
    ...
    for (final selection in selections) {
      for (final TextBox textBox in getBoxesForSelection(selection)) {
        final globalTopLeft = localToGlobal(Offset(textBox.left, textBox.top));
        final globalBottomRight =
            localToGlobal(Offset(textBox.right, textBox.bottom));
        rects.add(Rect.fromPoints(globalTopLeft, globalBottomRight));
      }
    }
    return rects;
  }
  ...
  void markNeedsLayout() {
    _lastSelectableFragments?.forEach(
        (SelectableFragment element) => element.didChangeParagraphLayout());
  ...
  set selectionColor(Color? value) {
    ...
    if (_lastSelectableFragments?.any(
            (SelectableFragment fragment) => fragment.value.hasSelection) ??
        false) {
      markNeedsPaint();
    }
  }
...
abstract mixin class TextSelectionGetterMixin {
  TextPosition? get textSelectionStart;
  TextPosition? get textSelectionEnd;
}
...
class SelectableFragment
    with Selectable, ChangeNotifier, TextSelectionGetterMixin
  ...
  TextPosition? get textSelectionStart => _textSelectionStart;
  TextPosition? get textSelectionEnd => _textSelectionEnd;
